...
const filterAllWithJobType = (filter, job, compName, lction) => {
...
const filterAllWithLevel = (filter, job, compName, lction) => {
...
const filterAllWithSalary = (filter, job, compName, lction) => {
...
const filterAllWithMarket = (filter, job, compName, lction) => {
...
export const filterJobs = (filter, job, compName, lction) => {
  if (filter.job_type.length > 0) {
    return filterAllWithJobType(filter, job, compName, lction);
  } else if (filter.job_seniority.length > 0) {
    return filterAllWithLevel(filter, job, compName, lction);
  } else if (filter.salary_range.length > 0) {
    return filterAllWithSalary(filter, job, compName, lction);
  } else if (filter.market.length > 0) {
    return filterAllWithMarket(filter, job, compName, lction);
  } else if (isAllFilled(filter)) {
    return filterWithAllFields(job, lction, compName, filter);
  } else if (
    filter.company_name &&
    filter.location &&
    job.location.match(lction) &&
